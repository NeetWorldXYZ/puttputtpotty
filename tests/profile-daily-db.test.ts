...
import migration from '../supabase/migrations/20260911231411_profile_daily_consistency.sql?raw';
...
const r=(await db.query<{wins: number; matches: number}>('select * from ranked_profile_record($1)',[user])).rows[0];
if(Number(r.wins)!==1 || Number(r.matches)!==2) throw Error('ranked aggregate incorrect');
const day=(await db.query<{day_key: string}>("select to_char(now() at time zone 'America/New_York','YYYY-MM-DD') as day_key")).rows[0].day_key;
async function add(seed: string,index: number){await db.query('insert into runs(user_id,course_seed,hole_index) values ($1,$2,$3)',[user,seed,index]);}
await add(day+'-pm',0);await add(day+'-pm',1);
for(const [seed,index] of [[day+'-pm',0],[day+'-am',2],[day+'-pm',9]] as const) {
