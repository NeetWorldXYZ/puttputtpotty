      ...
      <div className="kh-actions">
        ...
        <section className="kh-secondary" aria-label="More ways to play">
          <button className={`kh-card kh-daily${played ? ' kh-played' : ''}`} onClick={openDaily} disabled={!played && !current} aria-label={`Daily course. ${dailyDescription}`}>
            ...
            {played ? <span className="kh-daily-result"><b>{standing ? relative(standing.total - standing.par) : best}<small>{standing ? 'TO PAR' : 'STROKES'}</small></b><span><strong>{standing ? `#${standing.rank} ON THE BOARD` : 'VIEW RESULTS'}</strong><small>Next in {untilNext}</small></span></span> : <HomePodium/>}
          </button>
          <button className="kh-card kh-quick" onClick={() => go(() => navigate('match'), 'whoosh')}>
            <span className="kh-card-icon"><HomePutters/></span><span className="kh-card-copy"><strong>QUICK MATCH</strong><span>Ranked 1v1 · all time</span></span><span className="kh-chevron" aria-hidden="true">›</span>
            <span className="kh-record">{record ? <><span className="kh-record-wins"><b>{record.wins}</b><small>WINS</small></span><span className="kh-record-losses"><b>{record.losses}</b><small>LOSSES</small></span>{record.draws > 0 && <span><b>{record.draws}</b><small>DRAWS</small></span>}</> : <small>YOUR RANKED RECORD</small>}</span>
          </button>
        </section>
      </div>
    </main>
