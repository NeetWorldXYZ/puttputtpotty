...
type SpriteName = 'toilet' | 'plunger' | 'paper-roll' | 'sink' | 'plant' | 'janitor' | 'gum' | 'crown' | 'soap' | 'towel' | 'flange' | 'brush' | 'suitcase';
...
export function loadGameplaySprites(
  ...
  if (!loading) loading = Promise.all((['toilet', 'plunger', 'paper-roll', 'sink', 'plant', 'janitor', 'gum', 'crown', 'soap', 'towel', 'flange', 'brush', 'suitcase'] as const).map(async name => {
