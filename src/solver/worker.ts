...
import { generateCourse, generateHole, generateSlot, type CourseSlot, type GenerateOptions } from '../generator/generator';
...
export type WorkerRequest =
  ...
  | { kind: 'slot'; id: number; courseSeed: string; slot: CourseSlot; params: PhysicsParams };
...
self.onmessage = (e: MessageEvent<WorkerRequest>) => {
  ...
  try {
    ...
    } else if (req.kind === 'slot') {
      post({ id: req.id, slot: req.slot.index, generated: generateSlot(req.courseSeed, req.slot, req.params) });
    } else {
